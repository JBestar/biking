 ...
 function showHistory(arrBet){
    ...
    if(arrBet != null && arrBet.length > 0){
        ...
        for(var idx in arrBet){
            ...
            if(arrBet[idx].bet_date_from.length > 10 && arrBet[idx].bet_date_to.length > 10){
                tHtml += "<td>"+arrBet[idx].bet_date_from.substr(0, 10)+" ~ ";
                tHtml += arrBet[idx].bet_date_to.substr(0, 10)+"</td>";
            } else tHtml += "<td></td>"; 
            ...
            if(parseInt($("#stf_level").text()) >= 10) {
                tHtml += "<td><button name=\"delete\" value=\"" +arrBet[idx].bet_mb_uid + "\" ";
                tHtml += " from=\"" +arrBet[idx].bet_date_from.substr(0, 10) + "\" ";
                tHtml += " to=\"" +arrBet[idx].bet_date_to.substr(0, 10) + "\" ";
                tHtml += " >초기화</button></td>";
            } 
            tHtml += "</tr>";
        ...
        if(total_earn_sum >= 0){
            tSumHtml = "총금액합계: "+ total_earn_sum.toLocaleString() +"원";
 ...
 function addTbEventListner(){
    ...
    for(var idx = 0; idx < tblBtns.length; idx++){
        ...
        tblBtns[idx].addEventListener("click", function() {      
            ...
            if(this.name == "delete"){
                jsonData = { "mb_uid":this.value, "from":$(this).attr('from'), "to":$(this).attr('to')};
                requestDeleteBet(jsonData);
            } 
        });
