...
function getTdHtml(field, objSess) {
    ...
    switch(field){
        case "user_info":
            tHtml += "<td>";
            if (objSess.sess_mb_uid.length > 0) {
                tHtml += "<a href=\"/" + $('#category_name').text() + "/member?uid=" + objSess.sess_mb_uid;
                tHtml += "\" >" + objSess.sess_mb_uid + "</a>";
            }
        ...
        case "sess_run":
            tHtml +="<td>";
            tHtml += (parseInt(objSess.sess_running) == 1 ? "시작" : "정지");
            tHtml += "</td>";
            break;
        case "sess_time":
            tHtml +="<td>";
